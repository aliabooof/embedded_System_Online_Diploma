...
typedef struct{
	uint16_t 				EXTI_LineNumber;//For EXTI line form 0-15
...
typedef struct{
	...
	uint8_t 				Trigger_Case;/*Specifies the external Rising or Falling or Both.
 	 	   	   	   	   	   	   *This parameter must be set based on @ref EXTI_Trigger_define.*/

	uint8_t 				IRQ_EN;/*Enables or Disables the EXTI IRQ (That will enable the IRQ
									  *MASK in EXTI and also in the NVIC interrupt controller).
